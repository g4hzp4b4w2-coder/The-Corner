...
export default function DodgeMode({ lang, onBack, onSaveLiveSession }) {
  ...
  const handleSaveToJournal = async () => {
    ...
    const blocks = roundsHistory.map((r, i) => {
      ...
      return lang === "en"
        ? `${roundLabel}: ${r.hits} dodges, ${r.misses} missed, avg ${avg}`
        : `${roundLabel}: ${r.hits} kaçış, ${r.misses} kaçırıldı, ort. ${avg}`;
    });
    try {
      await onSaveLiveSession({ note, blocks, duration, threeMinRounds: 0, competes, type: "Kaçışlar" });
      setSaveStatus("saved");
    } catch {
      setSaveStatus("idle");
    }
  ...
  const startTraining = async () => {
    ...
    try {
      ...
      const loop = () => {
        ...
        // head visibly moves the same way on screen) — cheap to draw
        // twice, and keeps detection correctness completely separate
        // from display.
        ...
        if (currentPhase === "round") {
          ...
          if (targetRef.current) {
            ...
            ctx.strokeText(c(targetRef.current.key, lang), width - tx, ty);
            ctx.fillText(c(targetRef.current.key, lang), width - tx, ty);
          }
        }
